...
from .signal_engine_v2 import SignalEngine, get_bands_snapshot, bars_to_df
...
class TradingLoopV2:
    def __init__(
        ...
        *,
        ...
        is_blocked: Optional[Callable[[], Optional[str]]] = None,
    ...
    def _flatten_market(self, direction: str):
        ...
        try:
            ...
            order_id = data.get("orderId")
            self.log(f"Flatten order placed. orderId={order_id} tag={tag} side={side} size={size}")
        except Exception as e:
    ...
    def _compute_targets(self, direction: str, entry_price: float, entry_mode: str) -> tuple[float | None, list[float]]:
        ...
        if settings.get("tp_mid15") and s15:
            tp_prices.append(float(s15.mid))
        tp_prices.append(float(s5.upper) if direction == "LONG" else float(s5.lower))
    ...
    def start_signals(self):
        try:
            ...
            self.log(f"Start signals failed: {e}")
            return
        ...
        def on_signal(direction: str, payload: dict):
            ...
            trade = TradeState(
                ...
                sl_mode=settings.get("sl_mode", "Fixed ticks"),
                tick_size=float(settings.get("tick_size", 0.0)),
            )
        ...
        self._signal_engine = SignalEngine(
            ...
            tick_size=float(settings.get("tick_size", 0.1)),
            trend_filter_mode=str(settings.get("trend_filter_mode", "5m close vs midline")),
            ...
            while True:
                try:
                    ...
                    if time.time() - last > 15.0:
                        ...
                        contract_id = self.get_contract_id()
                        self._market.subscribe_contract(contract_id)
                        self.log(f"Live feed restarted. contractId={contract_id}")
                    time.sleep(1.0)
                except Exception:
                    time.sleep(1.0)
