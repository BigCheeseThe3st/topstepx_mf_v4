...
def get_bands_snapshot(
    ...
    *,
    ...
    # Use historical closed bars for band calculations (live=False),
    # matching the v5.1/practice model and Topstep chart behaviour.
    data = client.retrieve_bars(contract_id, tf, limit=limit, live=False, include_partial_bar=False)
    bars = data.get("bars") or data.get("candles") or data.get("data") or []
    if not bars:
        data = client.retrieve_bars(contract_id, tf, limit=limit, live=False, include_partial_bar=True)
        bars = data.get("bars") or data.get("candles") or data.get("data") or []
    if len(bars) < length:
    ...
    if df.empty or len(df) < length:
        return None
    last = df.iloc[-1]
    # Compute Bollinger Bands from CLOSE only (no typical price),
    # consistent with v5.1 and your Topstep chart settings.
    price = df["close"]
...
class SignalEngine:
    ...
    def __init__(
        ...
        self._logged_outside_window = False
    ...
    def get_current_snapshots(self) -> tuple[Optional[BandsSnapshot], Optional[BandsSnapshot]]:
        """
        Lightweight accessor for latest 1m/5m band snapshots.
        Used by v5 CLI to avoid re-querying the API during TP/SL calculation.
        """
        with self._snap_lock:
            return self._snap_1m, self._snap_5m
        ...
        while not self._stop.is_set():
            try:
                if self.trade_window_enabled and not _within_window_utc(self.trade_windows):
                    ...
                    if not self._logged_outside_window:
                        self._logged_outside_window = True
                        self.log(
                ...
                # We are inside the window again; allow next outside-window log in future.
                self._logged_outside_window = False
                ...
                # Periodic live debug to visualize trigger context (throttled for latency)
                now_ts = time.time()
                if now_ts - self._last_live_log_ts >= 5.0:
