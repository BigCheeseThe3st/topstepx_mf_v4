...
class TradingLoopV1:
    def __init__(
        ...
        *,
        ...
        # Throttle noisy "signal ignored" logs when signals keep firing.
        self._last_signal_ignored_log_ts: float = 0.0
        self._last_cooldown_log_ts: float = 0.0

        # Trade log CSV (millisecond timestamps for latency tracking).
        try:
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            self._trade_log_path = os.path.join(base_dir, "trades_log.csv")
        except Exception:
            self._trade_log_path = None

    # ---------- recovery ----------
    def recover_from_position(self, direction: str, entry_price: float):
        """Attach to an already-open position (e.g. after a crash). Recomputes TP/SL."""
        with self._trade_lock:
            if self._trade is not None:
                self.log("Recovery skipped: trade already active.")
                return
        try:
            entry_price = float(entry_price)
        except Exception:
            self.log("Recovery skipped: invalid entry_price.")
            return
        ...
        trade = TradeState(
            ...
            confirm_price=None,
            sl_mode=str(settings.get("sl_mode")),
        ...
        self.log(f"Recovery: attached to existing {direction} position at {entry_price:.2f}. Recomputing TP/SL...")

        def _recalc():
            try:
                sl, tps = self._compute_targets(trade.direction, trade.entry_price or 0.0, trade.entry_mode)
                if sl is None or not tps:
                    self.log("Recovery TP/SL calc failed; managing position with manual flatten only.")
                    return
                trade.sl_price = sl
                trade.tp_prices = tps
                self._set_trade(trade)
                ...
                self.log(f"Recovery targets set: SL={sl:.2f} TP={preview}")
            except Exception as e:
                self.log(f"Recovery TP/SL calc error: {e}")

        threading.Thread(target=_recalc, daemon=True).start()

    # ---------- trade logging ----------
    def _log_trade_event(self, event: str, trade: TradeState, **extra):
        """Append one trade event to trades_log.csv (millisecond timestamps for latency)."""
        try:
            ...
            row = {
                ...
                "entry_time": trade.entry_time.isoformat(timespec="milliseconds") if trade.entry_time else "",
                "sl_price": trade.sl_price,
                "tp_prices": ";".join(f"{p:.2f}" for p in (trade.tp_prices or [])),
                "entry_mode": trade.entry_mode,
            }
            for k, v in extra.items():
                row[k] = v
            fieldnames = [
                "timestamp_utc", "event", "direction", "status", "entry_price", "entry_time",
                "sl_price", "tp_prices", "entry_mode", "reason", "exit_price", "hit_tp_price",
                "live_band_price", "trigger_price",
            ]
    ...
    # ---------- broker sync ----------
    def sync_with_broker_position(self, has_position: bool, direction: str | None, entry_price: float | None):
        """
        Reconcile internal trade state with the broker's view (via UserHub).

        - If broker is flat but we still think we're in a trade, clear local state
          and log a CLOSE event with reason=EXTERNAL_EXIT.
        - If broker has an open position for this contract/account but we have no
          local trade, attach via recover_from_position so TP/SL and logging resume.
        """
        try:
            trade = self._get_trade()
            if not has_position:
                if trade is not None:
                    # Broker shows flat, but we still have a trade object. Treat as an
                    # external exit (e.g. brackets or manual close).
                    self.log("Broker flat but local trade active. Treating as EXTERNAL_EXIT and clearing state.")
                    try:
                        self._log_trade_event(
                            "CLOSE",
                            trade,
                            reason="EXTERNAL_EXIT",
                            exit_price=self._last_live_price if self._last_live_price is not None else "",
                        )
                    except Exception:
                        pass
            ...
            if trade is not None:
                # State already in sync: we have a trade and broker has position.
                # Only correct if direction matches (avoid any recalculation).
                if direction and trade.direction == direction:
                    return
            if trade is None and direction and entry_price is not None:
                self.log(
                    f"Broker shows open position but no local trade. "
                    f"Attaching via recovery. dir={direction} entry={entry_price:.2f}"
                )
                self.recover_from_position(direction=direction, entry_price=float(entry_price))
        except Exception as e:
            self.log(f"Broker sync error: {e}")
    ...
    def _place_live_order(self, direction: str, entry_price: float, sl_price: float, tp_prices: list[float]):
        ...
        try:
            ...
            # Do not clear the trade: keep managing TP/SL via quotes and avoid
            # accepting new signals or repeatedly retrying (e.g. "Not enough margin").
    ...
    def _compute_targets(self, direction: str, entry_price: float, entry_mode: str) -> tuple[float | None, list[float]]:
        ...
        # v5: Prefer reusing the latest 5m snapshot from the running SignalEngine
        # to avoid an extra HTTP round-trip during TP/SL calculation.
        s5 = None
        try:
            if hasattr(self, "_signal_engine") and self._signal_engine is not None:
                _, s5_cached = self._signal_engine.get_current_snapshots()  # type: ignore[attr-defined]
                s5 = s5_cached
        except Exception:
            s5 = None
        if not s5:
        ...
        def on_signal(direction: str, payload: dict):
            with self._trade_lock:
                ...
                if self._cooldown_active():
                    now_ts = time.time()
                    # Only log the cooldown block once per second at most
                    # to avoid spamming when signals fire rapidly.
                    if now_ts - self._last_cooldown_log_ts >= 1.0:
                ...
                if self._trade is not None or self._order_inflight or self._position_lock:
                    now_ts = time.time()
                    if now_ts - self._last_signal_ignored_log_ts >= 1.0:
                        self._last_signal_ignored_log_ts = now_ts
            ...
            self._log_trade_event("OPEN", trade, trigger_price=trigger_price)
        ...
        if hit_sl or hit_tp:
            ...
            try:
                self._log_trade_event(
                    ...
                    hit_tp_price=hit_tp_price if hit_tp_price is not None else "",
                    live_band_price=live_band_price if live_band_price is not None else "",
                )
            except Exception:
                pass
            if hit_sl:
