        ...
        while not self._stop.is_set():
            try:
                if self.trade_window_enabled and not _within_window_utc(self.trade_windows):
                    ...
                    continue
                if self.is_blocked:
                    reason = self.is_blocked()
                    if reason:
                        now_ts = time.time()
                        if now_ts - self._last_block_log_ts >= 30.0:
                            self._last_block_log_ts = now_ts
                            self.log(f"SignalEngine: blocked by calendar ({reason}).")
                        time.sleep(0.5)
                        continue
                ...
                # Trigger on band touch (edge) using live price at both 1m and 5m bands
                is_below = price <= s1.lower and price <= s5.lower
                is_above = price >= s1.upper and price >= s5.upper

                triggered = False
                direction = None
                trigger_price = None
                if is_below and not self._crossed_long:
                    direction = "LONG"
                    trigger_price = price
                    triggered = True
                if not is_below:
                ...
                if not triggered and is_above and not self._crossed_short:
                    direction = "SHORT"
                    trigger_price = price
                    triggered = True
                if not is_above:
